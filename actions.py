...
def create_alarm_from_tag(id, alarm_tag, instance_info, metric_dimensions_map, sns_topic_arn, alarm_separator,
    ...
    # Example: alarm_properties=['AutoAlarm', 'CWAgent', 'mem_used_percent', 'GreaterThanThreshold', '5m', 'Average', 'TMD_1', 'DP2EP_2of3']
    _TreatMissingDataKey = "TMD"
    _DP2EPKey = "DP2EP"
    dp2ep_m = None
    dp2ep_n = None
    ...
    if _TreatMissingData:
        ...
        temp_tmd = _TreatMissingData.split("_")[1] 

        if temp_tmd == "1":
            tmd_value = "breaching"
        elif temp_tmd == "2":
            tmd_value = "notBreaching"
        elif temp_tmd == "3":
            tmd_value = "ignore"
        elif temp_tmd == "4":
            tmd_value = "missing"

        logger.info(f"{tmd_value=}")

    if _DP2EP:
        dp2ep_value = _DP2EP.split("_")[1].split("of")
        dp2ep_m = int(dp2ep_value[0])
        dp2ep_n = int(dp2ep_value[1])
        logger.info(f"{dp2ep_m=}, {dp2ep_n=}")

    if dp2ep_n:
        EvaluationPeriods = dp2ep_n
    ...
    create_alarm(
        ...
        EvaluationPeriods, 
        sns_topic_arn,
        TreatMissingData=tmd_value,
        DatapointsToAlarm=dp2ep_m)
...
def create_alarm(AlarmName, AlarmDescription, MetricName, ComparisonOperator, Period, Threshold, Statistic, Namespace,
    ...
    # else:
    AlarmDescription = 'Created by cloudwatch-auto-alarms - lambda function'
    ...
    try:
        ...
        if DatapointsToAlarm:
            alarm["DatapointsToAlarm"] = DatapointsToAlarm
